...
def getClass(val_v, ar_a):
    V_and_A = []
    for ind_va in range(len(val_v)):
        V_and_A.append(val_v[ind_va] * 1 + ar_a[ind_va] * 2)
    return V_and_A
...
for file in files:
    ...
    temp_v_and_a = getClass(tmp_v, tmp_a)

    print(tmp_x.shape)
    print(dict(Counter(tmp_v)))
    print(dict(Counter(tmp_a)))
    print(dict(Counter(temp_v_and_a)))
    #
    # theta = []
    # alpha = []
    # beta = []
    ...
    print('V-A - all channel:')
    training(tmp_x, temp_v_and_a)
    X.append(tmp_x)
    YV += tmp_v
    YA += tmp_a
